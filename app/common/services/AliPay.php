...
class AliPay extends Pay
{
    ...
    public function alipayRefund2($refund_data, $set)
    {
        ...
        if(!empty($res)&&$res['alipay_trade_refund_response']['code'] == '10000'){
            ...
            \Log::debug('---alipay-pay---', $refund_data);
            \Log::debug('---alipay-pay---', $res);
    ...
    public function apprefund($refund_data)
    {
        ...
        if(!empty($res)&&$res['alipay_trade_refund_response']['code'] == '10000'){
            ...
            \Log::debug('---alipay-app---', $refund_data);
            \Log::debug('---alipay-app---', $res);
            throw new AppException($res['alipay_trade_refund_response']['msg'] . '-' . $res['alipay_trade_refund_response']['sub_msg']);
        }
    }
    ...
    public function withdrawCert($account, $name, $out_trade_no, $money)
    {
        ...
        $this->withdrawResult(['trade_no'=> $out_trade_no]);
     ...
     * @param $data
     */
    public function withdrawResult($param)
    {
        if (!empty($param)) {
            $pay_refund_model = \app\common\models\PayWithdrawOrder::getOrderInfo($param['trade_no']);

            if ($pay_refund_model) {
                $pay_refund_model->status = 2;
                $pay_refund_model->trade_no = $param['trade_no'];
                $pay_refund_model->save();
            }

            \app\common\services\finance\Withdraw::paySuccess($param['trade_no']);
            event(new \app\common\events\finance\AlipayWithdrawEvent($param['trade_no']));
        }
    }
