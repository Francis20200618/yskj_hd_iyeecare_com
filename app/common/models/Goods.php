...
class Goods extends BaseModel
{
    ...
    protected $appends = ['status_name','overseas'];
        ...
        foreach ($filters as $key => $value) {
            switch ($key) {
                ...
                case 'goods_id':
                    ...
                    break;
                case 'shop_type':
                    if($value == Marketing::SHOP_TYPE_JD){
                        $query->where('plugin_id', '=', 44);
                    }else{
                        $query->join('yz_ys_system_goods', function($join){
                            $join->on('yz_goods.id','=','yz_ys_system_goods.goods_id');
                        });
                        if ($value == Marketing::SHOP_TYPE_SELF) {
                            $query->where('yz_ys_system_goods.overseas', '!=', 1)->where('plugin_id', '!=', 44);
                        }elseif($value == Marketing::SHOP_TYPE_OVERSEAS){
                            $query->where('yz_ys_system_goods.overseas', '=', 1)->where('plugin_id', '!=', 44);
                        }
                    }
                    break;
                case 'act_or_where':
                    if (isset($value['category'])) {
                            ...
                            )->join('yz_goods_category', function ($join) {
                                return $join->on('yz_goods_category.goods_id', '=', 'yz_goods.id');
                            })->groupBy('yz_goods.id');
                        }
                    $query->where(function($query) use($value){
                        foreach ($value as $k => $v) {
                            switch ($k) {
                                case 'category':
                                    foreach ($v as $s) {
                                        $query->orWhereRaw('FIND_IN_SET(?,category_ids)', [$s]);
                                    }
                                    break;
                                case 'brand_id':
                                    $query->orWhereIn('brand_id', $v);
                                    break;
                                case 'id_in_list':
                                    $query->orWhereIn('yz_goods.id', $v);
                                    break;
                            }
                        }
                    });
                    break;
                case 'id_not_list':
                    $query->whereNotIn('yz_goods.id',$value);
                    break;
                case 'brand_id':
                    $query->where('brand_id', $value);
                    break;
...
//                return sprintf('%.2f', $this->deal_price - $price);
    ...
    public function ysGoods()
    {
        return $this->hasOne(YsGoods::class, 'goods_id', 'id');
    }

    public function getOverseasAttribute()
    {
        if ($this->ysGoods) {
            return $this->ysGoods->overseas;
        }
        return 0;
    }
    ...
    {
        $plugin = parent::getPlugin(); // TODO: Change the autogenerated stub
        if(is_null($plugin)){
            if ($this->overseas) {
                return app('plugins')->getPlugin('overseas');
            }
        }
        return $plugin;
    }
