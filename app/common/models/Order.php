...
class Order extends BaseModel
{
    ...
    {
        if (empty($plugin_ids)) {
            //酒店订单、租赁订单、网约车订单、服务站补货订单、拼团订单、拼购订单、抢团订单、聚合CPS订单、门店余额充值订单,圈仓订单(购买，提货),益生线下订单
            $plugin_ids = [33, 40, 41, 43, 54, 59, 69, 46, 70, 106, 96, 77, 78, 115, 74, 99, 39, 127 ,128,62];
        }
    ...
    public function refund()
    {
        if ($this->pay_type_id == 2) {
            ...
            if ($pay['alipay_pay_api'] == 0) {
                ...
                $errorData = [
                    'title' => '订单退款失败',
                    'content' => '订单：'.$this->id.'退款失败，支付宝老接口不支持快速退款通道',
                    'redirect_url' => '',
                    'redirect_param' => ''
                ];
        ...
        if (isset($result['batch_no'])) { //兼容支付宝老接口
            ...
            if (!$refundApply->save()) {
                ...
                $errorData = [
                    'title' => '后台申请退款失败',
                    'content' => '订单：'.$this->id.'后台申请退款写入数据库失败',
                    'redirect_url' => '',
                    'redirect_param' => ''
                ];
                $systemMsg->sendSysMsg(7,$errorData);
