...
class PreOrderGoods extends OrderGoods
{
    ...
    protected $hidden = ['goods', 'sale', 'belongsToGood', 'hasOneGoodsDispatch','marketingOrderGoods'];
    /**
     * @var PreOrder
     */
    public $order;
    ...
    protected $appends = ['pre_id', 'points', 'is_gift','gift_type','gift_price','marketings'];
    ...
    {
        $marketingService = $this->memberCart->marketingService;
        if (is_null($marketingService)) {
            return 0;
        }
        return $marketingService->giftType();
    }
    ...
    public function getMarketingsAttribute(){
        ...
        $marketingService = $this->memberCart->marketingService;
        if ($marketingService) {
            $result[] = $marketingService->marketing->only(['id','title']);
        }
        $discount_marketing = MarketingEnoughReduceService::getFirstOccurredDiscountMarketing($this->goods);
        if ($discount_marketing) {
            $result[] = $discount_marketing->only(['id','title']);
        }
        return $result;
    }
}