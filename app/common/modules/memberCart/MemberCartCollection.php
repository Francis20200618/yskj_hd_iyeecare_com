...
class MemberCartCollection extends Collection
{
    ...
    public function getTrade($member = null, $request = null)
    {
         ...
         * @var Trade $trade
         */
        $trade->init($this, $member, $request);//print_r($trade);exit();
        return $trade;
    }
...
//        }
        ...
        $every_is_gift = $orderGoodsCollection->every(function (PreOrderGoods $orderGoods) {
            return $orderGoods->is_gift;
        });
        if ($every_is_gift) {
            throw new AppException('营销活动赠品不可单独下单，请联系客服解决！');
        }
        /**
         * @var PreOrder $order
         */
