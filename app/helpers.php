...
if (!function_exists('uploadUrl')) {
    ...
    function uploadUrl($type = 'default')
    {
        if (config('app.framework') == 'platform') {
            ...
            $url['upload_url'] = './index.php?c=site&a=entry&m=yun_shop&do=shop&route=upload.upload.jsUpload&upload_type=';
            if ($type == 'new') {
                $image_url = str_replace('/web','.',yzWebUrl('upload.uploadImage.getImage'));
                $url['image_url'] = $image_url;
            } elseif($type == 'default') {
                $url['image_url'] = './index.php?c=site&a=entry&m=yun_shop&do=shop&route=upload.upload.jsImage';
            }
...
/**
 * 调试，打印调用栈
 * @param bool $print 是否直接打印 默认打印
 * @return string
 */
function debug_trace($print=true){
    ...
    if($trace){
        ...
        foreach($trace as $item){
            $rs .= $item['file'].':'.$item['line'].'  '
                .(isset($item['class']) ? $item['class']:'')
                . (isset($item['type']) ? $item['type']:'')
                .$item['function'].'('.implode(',', array_map('json_encode',$item['args'])).')  '.$end;
        }
    }
    if($print){
        echo $rs;exit;
