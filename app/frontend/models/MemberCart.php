 ...
 * @property MarketingService marketingService
...
class MemberCart extends \app\common\models\MemberCart
{
    ...
    public function getMarketingServiceAttribute()
    {
        if ($this->marketing_id) {
            $marketingService = new MarketingService(Marketing::find($this->marketing_id));
            if (!$marketingService->inActive()) {
                if ($this->id) {
                    $this->delete();
                }
                throw new ApiException('营销活动已结束');
            }
            return $marketingService;
        }
        return parent::getMarketingServiceAttribute();
    }
}
