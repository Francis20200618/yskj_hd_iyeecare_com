...
class PreOrder extends Order
{
    ...
    protected $appends = ['pre_id','marketings'];
    /**
    ...
    public function getMarketingsAttribute(){
        return array_values($this->getOccurredMarketings());
    }

    public function getOccurredMarketings(){
        $collect = [];
        foreach ($this->orderGoods as $orderGood) {
            if ($orderGood instanceof PreOrderGiftGoods) {
                continue;
            }
            if(is_null($orderGood->memberCart)){
                continue;
            }
            $marketing = MarketingService::getMarketingsInTrade($orderGood->memberCart);
            if ($marketing) {
                $collect[$marketing['id']] = $marketing;
            }
            $discount_marketing = MarketingService::getDiscountMarketingsInTrade($orderGood->memberCart);
            if ($discount_marketing) {
                $collect[$discount_marketing['id']] = $discount_marketing;
            }
        }
        return $collect;
    }
     ...
     * 获得赠品价格。
     */
    protected function getGiftPriceAttribute()
    {
    ...
    public function setOrderInvoiceperson()
    {

        $orderInvoiceperson = new PreOrderInvoiceperson();
        
        error_log(date("[Y-m-d H:i:s]")."\n\nuid:11111111111111111333333331\n\n", 3, "/log/invoice_".date("[Y-m-d]").".txt");

        $orderInvoiceperson->setOrder($this);
    }
    ...
    protected function initAttributes()
    {
        $attributes = array(
            ...
            'original_dispatch_price' => $this->getOriginalDispatchAmount(),//订单运费原价
...
//dd($this->priceCache);
     ...
     * @return int|number
     */
    public function getOriginalDispatchAmount()
    {

        return $this->getOrderDispatch()->_getFreight();
    }
