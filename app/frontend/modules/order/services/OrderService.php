...
class OrderService
{
    ...
    public static function getOrderGoods(Collection $memberCarts)
    {
        ...
        $result = $memberCarts->map(function ($memberCart) {
            ...
            if($memberCart->is_gift){
                $orderGoods = app('OrderManager')->make('PreOrderGiftGoods', $data);
            }else{
            ...
            $orderGoods->setRelation('memberCart', $memberCart);
            return $orderGoods;
        });
...
//        return $paySN;
    ...
    public static function ordersPay(array $param)
    {
        ...
        \Log::debug('---------订单支付ordersPay--------', ['orderPay'=>$orderPay->pay_type_id]);
