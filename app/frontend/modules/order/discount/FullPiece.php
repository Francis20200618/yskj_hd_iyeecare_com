...
class FullPiece extends BaseDiscount
{
    ...
    protected function _getAmount()
    {
        ...
        foreach ($settings['fullPiece'] as $k=>$v) {
            ...
            //赠品没有"满件优惠"
            foreach ($this->order->orderGoods->exceptGift() as $orderGoods) {
                //不可叠加的商品不算在内
                if (!MarketingService::inStackableActByGoods($orderGoods->goods, Marketing::STACKABLE_ACT_ENOUGH_NUM_REDUCE)) {
                    continue;
                }
                if(!MarketingEnoughReduceService::inStackableActByGoods($orderGoods->goods, $orderGoods->order, Marketing::STACKABLE_ACT_ENOUGH_NUM_REDUCE)){
                    continue;
                }
    ...
    private function totalAmount($fullPieces)
    {
        ...
        foreach ($fullPieces as $fullPiece) {
            ...
            //赠品没有"满件优惠"
            $goods_count = $this->order->orderGoods->exceptGift()->whereIn('goods_id', $fullPiece['goods'])->sum('total');

            $rules = collect($fullPiece['rules']);
            $rules = $rules->sortByDesc(function ($rule) {
                return $rule['enough'];
            });
            $amount = $this->order->orderGoods->exceptGift()->whereIn('goods_id', $fullPiece['goods'])->sum(function (PreOrderGoods $preOrderGoods) {
