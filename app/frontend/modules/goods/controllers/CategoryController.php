...
class CategoryController extends BaseController
{
    ...
    public function getGoodsList($category_id,$goods_page)
    {
        ...
        $goods_model = \app\common\modules\shop\ShopConfig::current()->get('goods.models.commodity_classification');
        $goods_model = new $goods_model;
		$goods_ids = DB::table('yz_goods_category')->select('goods_id')->whereRaw('FIND_IN_SET(?,category_ids)', [$category_id])->pluck('goods_id');

		$list = $goods_model
			->whereIn('id',$goods_ids)
			->uniacid()->WhereInPluginIds()->select(['yz_goods.id','yz_goods.title','yz_goods.thumb','yz_goods.market_price','yz_goods.price','yz_goods.cost_price','yz_goods.stock','yz_goods.real_sales','yz_goods.show_sales','yz_goods.virtual_sales','yz_goods.has_option', 'yz_goods.plugin_id'])
        ...
        foreach ($list as $goodsModel) {
            ...
            if ($goodsCategory[$goodsModel->id] && $goodsCategory[$goodsModel->id]['goods_option_id'] && $goodsCategory[$goodsModel->id]['goods_option']) {
                ...
                $goodsModel->offsetSet('goods_option_ids', explode('_',$goodsCategory[$goodsModel->id]['goods_option']['specs']));
                $goodsModel->thumb = yz_tomedia($goodsCategory[$goodsModel->id]['goods_option']['thumb']) ? : $goodsModel->thumb;
            }
