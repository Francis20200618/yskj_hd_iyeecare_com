...
class GoodsController extends ApiController
{
    ...
    public function searchGoods()
    {
        ...
        $build = $goods_model->SearchList($requestSearch)
            ->selectRaw(DB::getTablePrefix().'yz_goods.has_option,'.DB::getTablePrefix() ."yz_goods.id,thumb,real_sales+virtual_sales total_sales,market_price,price,cost_price,title,".DB::getTablePrefix() ."yz_goods.stock, " . DB::getTablePrefix() . "yz_goods.id as goods_id")
//            ->addSelect('yz_goods.has_option')
            ->with(['hasManyOptions' => function($query){
                $query->select('goods_id','product_price','market_price','stock');
            }])
...
//            if ($item->has_option && $item->hasManyOptions->isNotEmpty()) {
//                $item->min_price = $item->hasManyOptions->min("product_price");
//                $item->stock = $item->hasManyOptions->sum('stock');
//            }
        ...
        foreach ($list['data'] as &$v){
            if ($v['has_option'] && $v['has_many_options']){
                $v['min_price'] = min(array_column($v['has_many_options'],'product_price'));
...
//        }
    ...
    //route=goods.goods.getGoodsName
    public function getGoodsName()
    {
        $goods_id = intval(request()->goods_id);
        $uid = \YunShop::app()->getMemberId();
        $res = DB::table('yz_goods')->select('title','price')->find($goods_id);
        $res['uid'] = $uid;
        $res['nickname'] = DB::table('mc_members')->where('uid',$uid)->value('nickname');
        if ($res)
        {
