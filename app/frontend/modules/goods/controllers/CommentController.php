...
//            $list['favorable_rate'] = $this->favorableRate($goodsId);
    ...
    public function createComment()
    {
        ...
        if(request()->input('ysi')){
//            $dataModel = YsHelpEditData::uniacid()->find(request()->input('ysi'));
            $dataModel['is_helper'] = 1;
        }
        ...
        {
            $obj_or = $res->original;
            $arr_at = $obj_or['data']->attributes;
            DB::table('yz_member_confirm')->insert([
                'mid' => $member->uid,
                'did' => request()->input('ysi'),
                'tid' => $arr_at['id'],
                'plugin_type'=> 1,
                'confirm_a' => \YunShop::request()->confirm_a,
                'confirm_b' => \YunShop::request()->confirm_b,
                'created_at' => time()
            ]);
        }
...
//        $commentId = \YunShop::request()->comment_id;//评论主键ID 部分插件订单列表接口没有关联has_one_comment
        ...
        if ($comment) {
            ...
            $arrComment['level_name'] = $arrComment['has_one_member']['yz_member']['level']['level_name'] ?? \Setting::get('shop.member.level_name') ?? "普通会员";
    ...
    //goods.comment.get_praise_num 
    public function getPraiseNum()
    {
        $goods_detail_praise_num = DB::table('yz_comment_config')->orderBy('id','desc')->value('goods_detail_praise_num'); 
        return $this->successJson('获取成功',['praiseNum'=>$goods_detail_praise_num]);   
        //{"result":1,"msg":"\u83b7\u53d6\u6210\u529f","data":{"praiseNum":5}}  
    }
