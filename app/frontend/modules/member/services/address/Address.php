<?php
namespace app\frontend\modules\member\services\address;
class Address
{
    /*
    ** 智能解析
    */
    ...
    public static function compare()
    {
        ...
        require 'data/a2.php';
        require 'data/a1.php';
        foreach ($a1_data as $provinc_id=>$provinc) {
            $province = \app\common\models\member\Address::where('areaname',$provinc['name'])->where('level',1)->first();
            if (empty($province)) {
                echo 'province->'.$provinc['name']."<br/>";
                continue;
            }
            collect($a2_data)->where('pid', $provinc_id)->each(function($city,$city_id) use($a3_data,$province){
                $city_tmp = \app\common\models\member\Address::where('areaname', 'like', '%'.mb_substr($city['name'],0,-1).'%')->where('parentid', $province->id)->where('level',2)->first();
                if (empty($city_tmp) || $city_tmp->areaname != $city['name']) {
                    echo 'city->'.$city['name'].":".$city_tmp->areaname."<br/>";
                    return ;
                }
                collect($a3_data)->where('pid', $city_id)->each(function ($region, $region_id) use ($city_tmp) {
                    $region_tmp = \app\common\models\member\Address::where('areaname', 'like', '%'.mb_substr($region['name'],0,-1).'%')->where('parentid', $city_tmp->id)->where('level',3)->first();
                    if (empty($region_tmp) || $region_tmp->areaname != $region['name']) {
                        echo 'region->'.$region['name'].":".$region_tmp->areaname."<br/>";
                        return ;
                    }
                });
            });
