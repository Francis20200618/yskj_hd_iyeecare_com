...
class MemberCartController extends ApiController
{
    ...
    public function store()
    {
        ...
        if(empty($data['uniacid'])){
            throw new AppException('参数错误uniacid');
        }
        /**
...
//dd($cart_id);
    ...
    {
        $this->validate([
            'data.*.goods_id' => 'required|integer|min:0',
            'data.*.total' => 'required|integer|min:0',
            'data.*.option_id' => 'integer|min:0',
        ]);
        $data = request('data', []);
//        if(empty($data)){
//            return $this->errorJson('参数错误');
//        }
        $ids = MemberCart::where('member_id', \YunShop::app()->getMemberId())->where('marketing_id', request('marketing_id'))->where('is_gift', 1)->get()->pluck('id')->toArray();
        if ($ids) {
        ...
        foreach($data as $item){
            $memberCart = array(
                ...
                'is_gift' => $item['is_gift'] ? $item['is_gift'] : 0,
                'marketing_id' => $item['marketing_id'] ? $item['marketing_id'] : 0,
                'checked' => 1,
            );
            if(empty($memberCart['uniacid'])){
                throw new AppException('参数错误uniacid');
            }
            /**
             * @var MemberCart $cartModel
             */
            $cartModel = new MemberCart($memberCart);
            //验证商品是否存在购物车,存在则修改数量
            $cartModel->validate();
