...
abstract class CouponPrice
{
    ...
    protected function getOrderGoodsCollectionPaymentAmount()
    {
        return $this->coupon->getOrderGoodsInScope()->sum(function (PreOrderGoods $preOrderGoods){
            if(!MarketingService::inStackableActByGoods($preOrderGoods->goods, Marketing::STACKABLE_ACT_COUPON)){
                return 0;
            }
            if(!MarketingEnoughReduceService::inStackableActByGoods($preOrderGoods->goods, $preOrderGoods->order, Marketing::STACKABLE_ACT_COUPON)){
                return 0;
            }
    ...
    public function setOrderGoodsDiscountPrice()
    {
        ...
        $this->coupon->getOrderGoodsInScope()->map(function ($orderGoods) {
             ...
             */
            if(!MarketingService::inStackableActByGoods($orderGoods->goods, Marketing::STACKABLE_ACT_COUPON)){
                return;
            }
            if(!MarketingEnoughReduceService::inStackableActByGoods($orderGoods->goods, $orderGoods->order, Marketing::STACKABLE_ACT_COUPON)){
                return;
            }
