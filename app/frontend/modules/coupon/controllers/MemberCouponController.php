...
use Illuminate\Support\Facades\DB;//ShiJian
...
//    const TEMPLATEID = 'tqsXWjFgDGrlUmiOy0ci6VmVtjYxR7s-4BWtJX6jgeQ'; //临时调试用
    ...
    protected $ignoreAction = ['getCouponDetail'];
    protected $publicAction = ['getCouponDetail'];
...
//            $coupons[$k]['belongs_to_coupon']['discount'] = intval($coupons[$k]['belongs_to_coupon']['discount']);
    ...
    public function getCoupon()
    {
        ...
        $memberId = \YunShop::app()->getMemberId();//print_r($memberId);exit();
         ...
         */
        $couponModel = Coupon::find($couponId);//获取发票的详细信息 print_r($couponModel);exit();
        $now = date('Y-m-d H:i:s');
        if($couponModel->get_start_time != '0000-00-00 00:00:00' && $now < $couponModel->get_start_time){
            return $this->errorJson('优惠劵领取时间未开始！');
        }
        if($couponModel->get_end_time != '0000-00-00 00:00:00' && $now > $couponModel->get_end_time){
            return $this->errorJson('优惠劵领取时间已结束！');
        }
        $memberCoupon = (new PreMemberCoupon);
        $memberCoupon->init(Member::current(), $couponModel, $total);
        $memberCoupon->generate();//花落个人信息，点击的所有的优惠券信息，print_r($memberCoupon);exit();

        $member = Member::current()->yzMember;//花落个人信息 print_r($member);exit();
        //按前端要求, 需要返回和 couponsForMember() 方法完全一致的数据
        $coupon = Coupon::centerCouponsForMember($memberId, $member->level_id, $couponId)->get()->toArray();//print_r($coupon);exit();
    ...
    {        
        $id = \YunShop::request()->coupon_id;
        $res = \app\common\models\Coupon::find($id);
        if ($res['is_complex'] == 1) {
            $rule = '若订单满足优惠券使用条件，优惠券可多张叠加使用';
        } else {
            $rule = '每个订单本张优惠券仅限使用一张，若还满足其他优惠券使用条件，可叠加使用';
        ...
        $data = [            
            'name' => $res['name'],//优惠券名称
            'time_start' => $res->time_start->format('Y-m-d H:i:s'),//开始时间
            'time_end' => $res->time_end->format('Y-m-d H:i:s'),//结束时间
            'coupon_method' => $res['coupon_method'],//优惠券类型
            'enough' => $res['enough'],//满999可用的999
            'deduct' => $res['deduct'],//满999-600的600
            'discount' => $res['discount'],//折扣，9折的9
            'time_limit' => $res->time_limit,
            'time_days' => $res->time_days,
            'get_time_scope' => $res->get_time_scope,
            'coupon_type_name' => $this->getCouponTypeName($res['use_type']),
            'isIntegralExchange'=> $res['is_integral_exchange_coupon'],//折扣，9折的9
            'rule' => $rule//优惠券规则
        ];
    ...
    private function getCouponTypeName($useType)
    {
        switch ($useType) {
            case 0:
                $name = '平台商品';
