        ...
        $res->where('yz_coupon.get_type', '=', 1)
            ...
            ->where(function($query){
                $query->where(function($query){
                   $query->where('yz_coupon.get_start_time','0000-00-00 00:00:00')->orWhere('yz_coupon.get_end_time','0000-00-00 00:00:00');
                })->orWhere(function($query){
                    $now = date('Y-m-d H:i:s');
                    $query->where('yz_coupon.get_start_time','<=', $now)->where('yz_coupon.get_end_time', '>=', $now);
                });
            })
            // 优惠券的level_limit改为存储yz_member_level表的id，所以要关联yz_member_level表
            //->memberLevel($memberLevel);
