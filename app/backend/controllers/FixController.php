...
use Yunshop\Marketing\listeners\IntegralListener;
use Yunshop\Marketing\models\MarketingOrder;
use Yunshop\Marketing\models\MarketingOrderGoods;
use Yunshop\Marketing\service\MarketingGiftService;
...
class FixController extends BaseController
{
    ...
    public function test(){
        $memberCouponList = MemberCoupon::where('uid', 17)->where('coupon_id', 42)->where('used',0)->where('is_member_deleted',0)->orderBy('id','asc')->limit(2)->get();
        dd($memberCouponList->toArray());
        exit;
        $order = Order::find(1134);
//        $rs = MarketingGiftService::getIntegralAfterPaid($order);
        $rs = MarketingGiftService::removeIntegralOnRefund($order);
        dd($rs);
//        dd(collect([1,2,3])->contains(4));
//        $m = new MarketingOrderGoods();
//        var_dump($m->columns(),$m->hasColumn('order_goods_id'));exit;
//        event(new AfterOrderPaidEvent(Order::find(591)));
//        $marketingOrderList = MarketingOrder::whereRaw('FIND_IN_SET('.'591'.',order_ids)')->get();
//        dd($marketingOrderList);
        exit('ok');
    }
     ...
     * @return void
     */
    public function addYeePay(){
//        INSERT INTO `ims_yz_pay_type` (`id`, `name`, `plugin_id`, `code`, `type`, `unit`, `updated_at`, `created_at`, `deleted_at`, `setting_key`, `need_password`, `group_id`) VALUES
//        (88, '微信支付-易宝', 0, 'yeepay', 2, '元', 1640842860, NULL, NULL, '', 0, 0);
        $payType = new PayType();
        $payType->id = 88;
        $payType->name = '微信支付-易宝';
        $payType->plugin_id = 0;
        $payType->code = 'yeepay';
        $payType->type = 2;
        $payType->unit = '元';
    ...
    public function updateConfirm(){
        $rs = DB::table('yz_pay_type')->where('code','confirmPay')->update(['name'=>'资产抵扣']);
        dd($rs);
    }
