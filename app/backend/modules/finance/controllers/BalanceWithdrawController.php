...
class BalanceWithdrawController extends BaseController
{
    ...
    private function submitCheck()
    {
        if ($this->withdrawModel->status != 0) {
            $this->message('提交审核失败，该状态不能审核', yzWebUrl('finance.balance-withdraw.detail', ['id' => \YunShop::request()->id]), 'error');
        }
    ...
    public function submitPay()
    {
        ...
        if (in_array($this->withdrawModel->pay_way,['high_light_wechat','high_light_alipay','high_light_bank'])) {
...
//                $this->withdrawModel->pay_at = time();
                ...
                \Log::debug('高灯提现错误'.$this->withdrawModel->withdraw_sn,$result);
            }
        }
