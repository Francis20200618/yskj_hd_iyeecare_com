...
//dd($order->toArray());
    ...
    protected function getDivFrom($order)
    {
        ...
        $result['member_card'] = $order['has_many_member_certified']['idcard'] ?: $this->encryptIdCard($memberInfo->idcard);

        return $result;
    }

    protected function encryptIdCard($id_card){
        return substr($id_card, 0, 3).'************'.substr($id_card, -3);
    }
