...
class GoodsController extends UploadVerificationBaseController
{
    ...
    public function goodsSearch()
    {
        ...
        if ($producerId && app('plugins')->isEnabled('producer')) {
            $goodsBuild = Goodsselect(['id', 'display_order', 'thumb', 'title', 'has_option', 'price', 'stock', 'real_sales', 'status', 'is_hot', 'is_new', 'is_recommand', 'is_discount'])->Search($requestSearch)->pluginIdShow()->with(['hasOneSmallCodeUrl'])
...
//                ->paginate($per_size);
            ...
            $goodsBuild = Goods::select(['id', 'display_order', 'thumb', 'title', 'has_option', 'price', 'stock', 'real_sales', 'status', 'is_hot', 'is_new', 'is_recommand', 'is_discount'])->Search($requestSearch)->pluginIdShow()->with(['hasOneSmallCodeUrl']);
//                ->orderBy('yz_goods.display_order', 'desc')
//                ->orderBy('yz_goods.id', 'desc')
//                ->state($tab_state)
//                ->paginate($per_size);
...
//        $brands = Brand::getBrands()->getQuery()->select(['id','name'])->get()->toArray();
        ...
        if ($producerId && app('plugins')->isEnabled('producer')) {
            $list = Goods::select(['id', 'display_order', 'thumb', 'title', 'has_option', 'price', 'stock', 'real_sales', 'status', 'is_hot', 'is_new', 'is_recommand', 'is_discount'])->Search($requestSearch)->with(['hasOneSmallCodeUrl'])->pluginIdShow()
                ->whereHas('hasOneProducerGoods', function ($hasOneProducerGoods) use ($producerId) {
                    $hasOneProducerGoods->where('producer_id', $producerId);
                })
            ...
            $list = Goods::select(['id', 'display_order', 'thumb', 'title', 'has_option', 'price', 'stock', 'real_sales', 'status', 'is_hot', 'is_new', 'is_recommand', 'is_discount'])
...
//        return plugin_assets('','views/widget/');
        if (request()->ajax()) {

//            dd(request()->input('widgets'));
//            dd(request()->input());

            $id = request()->input('id');
            $goods_service = new EditGoodsService($id, \YunShop::request());
...
//        });
    ...
    public function getSearchMemberTagsJson()
    {
        if (app('plugins')->isEnabled('member-tags')) {
            $keyword = request()->keyword;
            $tags = MemberTagsModel::GetTagsByTitle($keyword);
