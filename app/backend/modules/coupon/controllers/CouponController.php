...
class CouponController extends BaseController
{
    ...
    public function couponData()
    {
        ...
        foreach ($list['data'] as &$item) {
            ...
            $item['usetotal'] = MemberCoupon::uniacid()->where("coupon_id", $item['id'])->where("used", 1)->count();
            // 总数为负数代表无限张,无需计算
            $lasttotal = $item['total'] < 0 ? $item['total'] : $item['total'] - $item['gettotal'];
            $item['lasttotal'] = $lasttotal; //考虑到可领取总数修改成比之前的设置小, 则会变成负数
        }
    ...
    {
        $keyword = request()->keyword;
        $list = Coupon::getCouponsBySearch($keyword)
            ->pluginId()
            ->orderBy('display_order', 'desc')
            ->orderBy('updated_at', 'desc')
            ->get();
    ...
    public function queryById(){
        $ids = request()->ids;
        $list = Coupon::whereIn('id', explode(',', $ids))
            ->pluginId()
            ->orderBy('display_order', 'desc')
            ->orderBy('updated_at', 'desc')
            ->get();
...
//            });
        ...
        $goods_ids = is_array($coupon['use_conditions']['good_ids'])?$coupon['use_conditions']['good_ids']:[];
        $store_ids = is_array($coupon['use_conditions']['store_ids'])?$coupon['use_conditions']['store_ids']:[];
        $store_is_open = app('plugins')->isEnabled('store-cashier');
        ...
        $data = [
            ...
            'store_is_open' => $store_is_open,
            'goods' => Goods::select(['id', 'title'])->uniacid()->whereIn('id', $goods_ids)->get(),
            'store' => app('plugins')->isEnabled('store-cashier') ? Store::select(['id', 'store_name'])->uniacid()->whereIn('id', $store_ids)->get() : [],
...
//        ])->render();
    ...
    {
        $i['i'] = \YunShop::app()->uniacid;
        return $this->successJson('ok',$i);
    }
}
