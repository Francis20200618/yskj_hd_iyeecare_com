  ...
  <div id="app">
    <div class="vue-head goods-page_header">
      <div class="goods-page_header-buttons">
        <el-button type="text" :class="currentShowPage==pageItem.key?'goods-page_header-current-button':''" v-for="pageItem in pages" :key="pageItem.key" @click="currentShowPage=pageItem.key">[[ pageItem.title ]]</el-button>
      </div>
      <div class="tabs">
        <!-- 韦总说要换行显示 -->
        <div v-for="subPageItem in subPages" :key="subPageItem.name" class="tab-item" @click.stop="chooseTab(subPageItem)" :class="{'active': showComponentName == subPageItem.componentName}">
          [[subPageItem.title]]
        </div>
      </div>
        ...
        <li v-for="pageArray in pages" :key="pageArray.key">
          <ul>
            <li v-for="page in pageArray.childrens" :key="page.path">
              <component :ref="page.componentName" :is="page.componentName" :http_url="http_url" :form="page['data']" :attr_hide="page['attr_hide']" v-if="$options.components[page.componentName]" v-show="showComponentName==page.componentName"></component>
            </li>
          </ul>
        </li>
      </ul>
