...
@section('content')
    ...
    <div id='re_content' >
        <div class="vue-crumbs">
            <a @click="goBack">系统</a> > 微信模板消息 > 自定义模板管理 > 添加模板
        </div>
        <div class="con">
            <div class="setting">
                <div class="block">
                    ...
                    <el-form ref="form" :model="form" label-width="20%">
                        <div class="left">
                            ...
                            <el-form-item label="选择模板">
                                <template>
                                    <el-select style="width:90%;" filterable placeholder="请选择" v-model="form.template_id" @change="Select" filterable>
                                        <el-option
                            ...
                            <el-form-item label="模板展示" v-if="content">
                                <div style="width: 90%;min-height:129px;background-color: #f6f7f9;border-radius: 4px;box-sizing:border-box;padding:8px;" v-html="content">
                                </div>
                            </el-form-item>
                            <el-form-item label="模板变量类型" >
                                <template>
                                    <el-select style="width:90%;" filterable placeholder="请选择" v-model="value" @change="paramSelect" filterable>
                                        <el-option
                                                v-for="item in temp"
                                                :label="item.title"
                                                :value="item.value">
    ...
    <script>
        var vm = new Vue({
            ...
            methods: {
                ...
                getData(){
                    this.$http.post('{!! yzWebFullUrl('setting.diy-temp.add') !!}').then(function (response){
                        if (response.data.result) {
                ...
                submit() {
                    ...
                    this.$http.post('{!! yzWebFullUrl('setting.diy-temp.add') !!}',{'temp':this.form}).then(function (response){
                        this.$message({message: response.data.msg,type: 'success'});
