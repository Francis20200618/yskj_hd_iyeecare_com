...
<div class="all">
    <div id="qrcode" ref="qrcode" class="qrcode"  style="display:none;"></div>
    <div id="app" v-cloak>
        ...
        <div class="vue-main">
            <div class="vue-main-form">
                <el-table :data="list" style="width: 100%">
                    ...
                    <el-table-column label="是否可领取" align="center" width="80">
                        <template slot-scope="scope">
                            <div>
                                <div>[[scope.row.get_type==0?'不可以':'可以']]</div>
                            </div>
                        </template>
                    </el-table-column>
                    <el-table-column label="是否在领券中心显示" align="center" width="80">
                        <template slot-scope="scope">
                            <div>
                                <div>[[scope.row.is_display==0?'显示':'不显示']]</div>
                            </div>
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column prop="refund_time" label="操作" align="center" width="300">
                        <template slot-scope="scope">
                            ...
                            <!--优惠券 链接和二维码-->
                            <el-link title="下载二维码" :underline="false" style="width:50px;color:#666;" >
                                <a  class="iconfont icon-ht_operation_ewecode"   href="" download style="color:#666;" @click="copy_ewecode(scope.row.id,$event)"></a>
                            </el-link>
                            <el-link title="复制链接" :underline="false" style="width:50px;">
                                <i  class="iconfont icon-ht_operation_copyline" :data-clipboard-text="copy_copyline_text"  @click="copy_copyline(scope.row.id,$event)"
...
<div data-clipboard-text="" class="copyTip" style="display: none;"> 提示</div>
...
<script>
    var app = new Vue({
        ...
        mounted() {
            this.getData(1);
            this.clipboard = new Clipboard(".copyTip");
        },
        methods: {  
            copy_ewecode(coupon_id,e){
                  //console.log(e.target);
                  let strClass ='icon-ht_operation_ewecode'+coupon_id;
                  $(e.target).addClass(strClass);
                  let url = 'https://hd.iyeecare.com/addons/yun_shop/?menu#/coupon/get_coupon_from/'+coupon_id+'?i=2';
                ...
                $('.'+strClass+'').attr('href',data);
                $('.'+strClass+'').attr('download','优惠券id'+coupon_id+'二维码图片');
                
            },
            copy_copyline(coupon_id,e){
                let strClass ='icon-ht_operation_copyline'+coupon_id;
                  $(e.target).addClass(strClass);

                var data= 'https://hd.iyeecare.com/addons/yun_shop/?menu#/coupon/get_coupon_from/'+coupon_id+'?i=2';
                $('.'+strClass+'').attr('data-clipboard-text',data);
                this.clipboard = new Clipboard('.'+strClass);
                //alert('ok 1');
                 this.clipboard.on("success", e => {
                  this.$message({type: 'success',message: '复制成功,注意：优惠券设置为可领取状态'});
                   // 释放内存
